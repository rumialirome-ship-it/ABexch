...
const ApproveCommissionsPage: React.FC = () => {
    ...
    return (
        <MainLayout title="Approve Commissions" showBackButton titleClassName="text-accent-tertiary text-shadow-glow-tertiary">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full bg-transparent rounded-lg">
                        <thead className="border-b-2 border-border-color">
                            <tr>
                                <th className="py-3 px-4 text-left text-accent-tertiary font-semibold tracking-wider uppercase text-sm text-shadow-glow-tertiary">Recipient ID</th>
                                <th className="py-3 px-4 text-left text-accent-tertiary font-semibold tracking-wider uppercase text-sm text-shadow-glow-tertiary">Type</th>
                        ...
                        <tbody>
                            {commissions.map((c) => (
                                <tr key={c.id} className="border-b border-border-color/50 last:border-b-0 hover:bg-accent-tertiary/5 transition-colors duration-300">
                                    <td className="py-3 px-4 font-mono">{c.recipientId}</td>
                                    <td className="py-3 px-4 capitalize">{c.recipientType}</td>
                                    <td className="py-3 px-4">{c.drawLabel}</td>
                                    <td className="py-3 px-4 text-right font-mono">{formatCurrency(c.amount)}</td>
                                    <td className="py-3 px-4 text-center">
                                        <button onClick={() => handleApprove(c.id)} className="bg-accent-primary/80 text-black font-bold py-1 px-3 rounded text-sm transition-all duration-300 hover:bg-accent-primary hover:-translate-y-0.5 active:scale-95">
                                            Approve
                                        </button>
                                    </td>
                                </tr>
